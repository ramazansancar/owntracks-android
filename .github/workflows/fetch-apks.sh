...
MATCHING_PIPELINE_ID=$(echo $PIPELINE_OUTPUT | jq -r '.items[] | select(.vcs.revision == env.HEAD_SHA and .vcs.branch == "master") |.id')
echo "Pipeline ID that matches git rev $HEAD_SHA is $MATCHING_PIPELINE"
